...
async function performEnhancedUnifiedSearch(
  ...
  if (types.includes('all') || types.includes('knowledge')) {
    try {
      ...
      // Build dynamic query for expanded search
      const whereConditions = searchPatterns.map(() => 'title LIKE ? OR content LIKE ?').join(' OR ');
      const searchParams = searchPatterns.flatMap(pattern => [pattern, pattern]);
      ...
      const knowledgeDocs = await db.all(`
        ...
        LIMIT ?
      `, [...searchParams, Math.floor(limit / 6)]);
    ...
    if (includeChunks) {
      const chunkWhereConditions = searchPatterns.map(() => 'c.chunk_content LIKE ?').join(' OR ');
      const chunks = await db.all(`
        ...
        LIMIT ?
      `, [...searchPatterns, Math.floor(limit / 10)]);
  ...
  if (types.includes('all') || types.includes('business_case')) {
    try {
      ...
      const bcWhereConditions = searchPatterns.map(() => 'title LIKE ? OR summary LIKE ? OR challenge LIKE ? OR solution LIKE ?').join(' OR ');
      const bcSearchParams = searchPatterns.flatMap(pattern => [pattern, pattern, pattern, pattern]);
      ...
      const businessCases = await db.all(`
        ...
        ORDER BY updated_at DESC 
        LIMIT ?
      `, [...bcSearchParams, Math.floor(limit / 6)]);
  ...
  if (types.includes('all') || types.includes('tool')) {
    ...
    const toolWhereConditions = searchPatterns.map(() => 'title LIKE ? OR description LIKE ? OR url LIKE ?').join(' OR ');
    const toolSearchParams = searchPatterns.flatMap(pattern => [pattern, pattern, pattern]);
    
    const tools = await db.all(`
      SELECT 
        c.id, c.title, c.description, c.url, c.tags, c.themes, c.topics,
        c.created_at, c.updated_at
      FROM content c
      JOIN tools t ON c.id = t.id
      WHERE c.content_type = 'tool' AND (${toolWhereConditions.replace(/title/g, 'c.title').replace(/description/g, 'c.description').replace(/url/g, 'c.url')})
      ORDER BY c.updated_at DESC 
      LIMIT ?
    `, [...toolSearchParams, Math.floor(limit / 6)]);
  ...
  if (types.includes('all') || types.includes('hoodie')) {
    ...
    const hoodieWhereConditions = searchPatterns.map(() => 'name LIKE ? OR description LIKE ? OR category LIKE ? OR subcategory LIKE ?').join(' OR ');
    const hoodieSearchParams = searchPatterns.flatMap(pattern => [pattern, pattern, pattern, pattern]);
    ...
    const hoodies = await db.all(`
      ...
      LIMIT ?
    `, [...hoodieSearchParams, Math.floor(limit / 6)]);
  ...
  if (types.includes('all') || types.includes('video')) {
    ...
    const videoWhereConditions = searchPatterns.map(() => 'title LIKE ? OR description LIKE ?').join(' OR ');
    const videoSearchParams = searchPatterns.flatMap(pattern => [pattern, pattern]);
    ...
    const videos = await db.all(`
      ...
      ORDER BY updated_at DESC 
      LIMIT ?
    `, [...videoSearchParams, Math.floor(limit / 7)]);
  ...
  if (types.includes('all') || types.includes('newsletter')) {
    ...
    const newsletterWhereConditions = searchPatterns.map(() => 'title LIKE ? OR description LIKE ?').join(' OR ');
    const newsletterSearchParams = searchPatterns.flatMap(pattern => [pattern, pattern]);
    ...
    const newsletters = await db.all(`
      ...
      ORDER BY updated_at DESC 
      LIMIT ?
    `, [...newsletterSearchParams, Math.floor(limit / 7)]);
  ...
  if (types.includes('all') || types.includes('content')) {
    ...
    const contentWhereConditions = searchPatterns.map(() => 'title LIKE ? OR description LIKE ?').join(' OR ');
    const contentSearchParams = searchPatterns.flatMap(pattern => [pattern, pattern]);
    ...
    const content = await db.all(`
      ...
      ORDER BY updated_at DESC 
      LIMIT ?
    `, [...contentSearchParams, Math.floor(limit / 7)]);
  ...
  console.log(`🧠 Applying search intelligence to ${allResults.length} results...`);
  ...
  const enhancedResults: EnhancedSearchResult[] = await Promise.all(
    allResults.map(async (result) => {
      ...
      const relationships = detectContentRelationships(result, allResults);
